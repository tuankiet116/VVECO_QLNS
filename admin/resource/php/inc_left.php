	...
	foreach($arrayModule as $key=>$row){
		...
		if(file_exists("modules/" . $row["mod_path"] . "/inc_security.php")===true){
			...
			<li id="listItem_<?=$row["mod_id"]?>">
				<div class="abcde">
                    <span class="glyphicon glyphicon-cog" style="cursor:pointer; font-size: 14px; color: #FFFFFF; top:2px" id="image_<?=$i?>" onclick="showhidden(<?=$i?>);" title="<?=translate_text("Show list menu")?>" ></span>&nbsp;
                    <span style="cursor:pointer" onclick="showhidden(<?=$i?>);"><?=$row["mod_name"]?></span>
				...
				<table cellpadding="5" cellspacing="0" width="100%" class="table table_menu">
					...
					foreach($arraySub as $key=>$value){
						...
						<tr>
							...
							<td colspan="2" class="m"><span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>&nbsp;<a class="tab" id="<?=$iTab?>" rel="<?=$row["mod_name"]?><span class='raquo'>&raquo;</span><?=$value?>" onclick="return false;" target="_blank" href="modules/<?=$row["mod_path"]?>/<?=$url?>"><?=trim($value)?></a></td>
						</tr>
					<?
					}
					?>
